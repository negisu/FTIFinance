Public Class frmFINDocumentType
    ...
    Private FIELD_CODE_DEPT As String = "_CODE_DEPT"
    Private FIELD_NAME_DEPT As String = "_NAME_DEPT"

    Private FIELD_CONTACT_NAME_THA As String = "_CONTACT_NAME_THA"
    Private FIELD_CONTACT_NAME_ENG As String = "_CONTACT_NAME_ENG"

    Private FIELD_IS_TRANSFER As String = "_IS_TRANSFER"
    Private FIELD_IS_CUT_DEPT As String = "_IS_CUT_DEPT"

    Private TABLE_NAME As String = "DOCUMENT_TYPE"

    Private Sub frmFINDocumentType_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ds = New DataSet
    ...
    Private Sub GetList()
        ...
        If Not String.IsNullOrEmpty(searchValue) Then
            ...
            If cbSearch.SelectedIndex = 1 Then
                qRep &= String.Format(" WHERE (" + FIELD_CODE_DEPT + " LIKE '%{0}%')", searchValue)
            End If
            If cbSearch.SelectedIndex = 2 Then
                qRep &= String.Format(" WHERE (" + FIELD_NAME + " LIKE '%{0}%')", searchValue)
            End If
            If cbSearch.SelectedIndex = 3 Then
                qRep &= String.Format(" WHERE (" + FIELD_NAME_DEPT + " LIKE '%{0}%')", searchValue)
            End If
            If cbSearch.SelectedIndex = 4 Then
                qRep &= String.Format(" WHERE (" + FIELD_CONTACT_NAME_THA + " LIKE '%{0}%') OR (" + FIELD_CONTACT_NAME_ENG + " LIKE '%{0}%')", searchValue)
            End If
        End If
    ...
    Private Sub btnAdd_Click(sender As Object, e As EventArgs) Handles btnAdd.Click
        If txt1.TextLength > 0 And txt2.TextLength > 0 And txt3.TextLength > 0 And txt5.TextLength > 0 Then
            ...
            If CNT = 0 Then
                ...
                parameters.Add("@p4", txt5.Text)
                parameters.Add("@p5", txt6.Text)

                parameters.Add("@p6", If(chkY1.Checked, "Y", "N"))
                parameters.Add("@p7", If(chkY2.Checked, "Y", "N"))
                qRep = String.Empty
                qRep = "INSERT INTO " + TABLE_NAME + " (" + FIELD_CODE + ", " + FIELD_CODE_DEPT + ", " + FIELD_NAME + ", " + FIELD_NAME_DEPT + ", " + FIELD_CONTACT_NAME_THA + ", " + FIELD_CONTACT_NAME_ENG + ", " + FIELD_IS_TRANSFER + ", " + FIELD_IS_CUT_DEPT + ") VALUES (@p0,@p1,@p2,@p3,@p4,@p5,@p6,@p7)"
