Public Class frmFINInvoice
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        If isNew Then
            TRAN_NO = DocumentNumberHelper.getPN_DOC_RUNNING(OU_CODE, user_div, DateTime.Now.ToString("yyyy", New System.Globalization.CultureInfo("th-TH").DateTimeFormat), "IV1")
        End If
    ...
    Private Sub frmFINPaymentNotice_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
        If Not isLockAll Then
            If isNew Then
                ...
                End If
            Else
                If (MessageBox.Show("การแก้ไขใบแจ้งหนี้ยังไม่ถูกบันทึก คุณต้องการที่จะปิดหน้าต่างนี้ ใช่หรือไม่?", "", MessageBoxButtons.YesNo) = Windows.Forms.DialogResult.No) Then
                    e.Cancel = True
                End If
            End If
        End If
