Public Class frmFINFormRC
    ...
    Dim REF_RCDatatable As DataTable
    ...
    Private Sub gridViewInit()
        ...
        Dim query As String = "SELECT TOP 0 * FROM FIN_PN_IV_DETAIL INNER JOIN IV_SUB_SECTION ON FIN_PN_IV_DETAIL.SUB_SECTION_CODE = IV_SUB_SECTION.SUB_SECTION_CODE INNER JOIN SU_DIVISION ON IV_SUB_SECTION.DIV_CODE_INC = SU_DIVISION.DIV_CODE "
        DETAILDataTable = fillWebSQL(query, Nothing, "FIN_PN_IV_DETAIL")
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        If validateFIN_PN_IV_DETAIL() And validateAddress() Then
            If (MessageBox.Show("คุณต้องการที่จะบันทึกเอกสารใช่หรือไม่?", "บันทึกเอกสาร", MessageBoxButtons.YesNo) = Windows.Forms.DialogResult.Yes) Then
                If isNew Then
                    TRAN_NO = DocumentNumberHelper.getDOC_RUNNING(OU_CODE, user_div, DateTime.Now.ToString("yyyy", New System.Globalization.CultureInfo("th-TH").DateTimeFormat), TRAN_TYPE, DateTime.Now.ToString("MM"))
...
ByVal e As DataGridViewDataErrorEventArgs) Handles DetailGridView.DataError
    ...
    Private Sub pickAR(sender As Object, e As EventArgs)
        ...
        If f.ShowDialog(Me) = Windows.Forms.DialogResult.OK Then
            ...
            AR_NAMETextBox.Text = selectedRow.Cells("PRENAME_FORMAT_TH").Value.ToString().Replace("{0}", selectedRow.Cells("COMP_PERSON_NAME_TH").Value.ToString())
            AR_NAME_ENTextBox.Text = selectedRow.Cells("PRENAME_FORMAT_EN").Value.ToString().Replace("{0}", selectedRow.Cells("COMP_PERSON_NAME_EN").Value.ToString())
            ...
            Try
                ...
                'add branch later
                BRANCH_NAME_ENTextBox.Text = mailSelectedRow.Cells("ADDR_NOTE").Value.ToString()
                BRANCH_NAME_THTextBox.Text = mailSelectedRow.Cells("ADDR_NOTE_EN").Value.ToString()
            Catch ex As Exception
                'resetAddress()
            End Try
    ...
    Private Sub resetAddress()
        ...
        BRANCH_NAME_ENTextBox.Text = String.Empty
        BRANCH_NAME_THTextBox.Text = String.Empty
    End Sub
    ...
    Private Sub getHEADDatatable()
        ...
        query &= "SELECT  TOP 1 * "
        query &= "FROM            FIN_PN_IV_HEAD LEFT JOIN MB_COMP_PERSON ON FIN_PN_IV_HEAD.AR_CODE = MB_COMP_PERSON.COMP_PERSON_CODE LEFT JOIN MB_PRENAME ON MB_COMP_PERSON.PREN_CODE = MB_PRENAME.PRENAME_CODE "
        query &= "WHERE           TRAN_NO = @p0"

        parameters.Add("@p0", TRAN_NO)

        HEADDataTable = fillWebSQL(query, parameters, "FIN_PN_IV_HEAD")
        If isNew Then
    ...
    Private Sub getHEAD()
        ...
        If HEADDataTable.Rows(0).Item("CANCEL_FLAG").ToString() = "N" Then
            DocumentStatusLabel.Text = "ปกติ"
        Else
        ...
        AR_NAMETextBox.Text = HEADDataTable.Rows(0).Item("COMP_PERSON_NAME_TH").ToString()
        AR_NAME_ENTextBox.Text = HEADDataTable.Rows(0).Item("COMP_PERSON_NAME_EN").ToString()
    ...
    Private Sub getADDRESS()
        ...
        BRANCH_NAME_ENTextBox.Text = HEADDataTable.Rows(0).Item("BRANCH_NAME_EN").ToString()
        ADDR1_EN.Text = HEADDataTable.Rows(0).Item("ADDR1_EN").ToString()
        ADDR1_TH.Text = HEADDataTable.Rows(0).Item("ADDR1_TH").ToString()
        ATTN_NAME_ENTextBox2.Text = HEADDataTable.Rows(0).Item("ATTN_NAME_EN").ToString()
        ADDR1_EN2.Text = HEADDataTable.Rows(0).Item("ADDR2_EN").ToString()
        ATTN_NAME_THTextBox2.Text = HEADDataTable.Rows(0).Item("ATTN_NAME_TH").ToString()
        ADDR1_TH2.Text = HEADDataTable.Rows(0).Item("ADDR2_TH").ToString()
        POST_CODETextBox2.Text = HEADDataTable.Rows(0).Item("POST_CODE").ToString()
        TELEPHONETextBox2.Text = HEADDataTable.Rows(0).Item("TELEPHONE1").ToString()
        FAXTextBox2.Text = HEADDataTable.Rows(0).Item("FAX1").ToString()
        TELEPHONETextBox2.Text = HEADDataTable.Rows(0).Item("TELEPHONE2").ToString()
        FAXTextBox2.Text = HEADDataTable.Rows(0).Item("FAX2").ToString()
    End Sub

    Private Sub getDETAIL()
        Dim query As String = "SELECT * FROM FIN_PN_IV_DETAIL INNER JOIN IV_SUB_SECTION ON FIN_PN_IV_DETAIL.SUB_SECTION_CODE = IV_SUB_SECTION.SUB_SECTION_CODE INNER JOIN SU_DIVISION ON IV_SUB_SECTION.DIV_CODE_INC=SU_DIVISION.DIV_CODE  WHERE TRAN_NO = @p0 ORDER BY SEQ"
        Dim parameters As New Dictionary(Of String, Object)
        parameters.Add("@p0", TRAN_NO)
        DETAILDataTable = fillWebSQL(query, parameters, "FIN_PN_IV_DETAIL")
        DetailGridView.DataSource = DETAILDataTable
        DetailGridView.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.AllCells
        DetailGridView.AutoResizeColumns()
        DetailGridView.Refresh()
    ...
    Private Sub insertOrUpdateHEAD()
        Dim parameters As New Dictionary(Of String, Object)
        Dim query As String = "DELETE FROM FIN_PN_IV_HEAD WHERE TRAN_NO = @p0"
        parameters.Add("@p0", TRAN_NO)
        Try
        ...
        pn.ADDR2_EN = ADDR1_EN2.Text
        pn.ADDR2_TH = ADDR1_TH2.Text
        pn.ATTN_NAME_EN = ATTN_NAME_ENTextBox2.Text
        pn.ATTN_NAME_TH = ATTN_NAME_THTextBox2.Text
        pn.POST_CODE = POST_CODETextBox.Text
        pn.TELEPHONE1 = TELEPHONETextBox.Text
        pn.FAX1 = FAXTextBox.Text
        pn.TELEPHONE2 = TELEPHONETextBox2.Text
        pn.FAX2 = FAXTextBox2.Text
        pn.AR_CODE = AR_CODETextBox.Text
        pn.BRANCH_NAME_TH = BRANCH_NAME_THTextBox.Text
        pn.BRANCH_NAME_EN = BRANCH_NAME_ENTextBox.Text
        pn.COMP_PERSON_NAME_TH = AR_NAMETextBox.Text
        pn.COMP_PERSON_NAME_EN = AR_NAME_ENTextBox.Text
        pn.CANCEL_FLAG = "N"
        pn.PROG_ID = Me.Name
    ...
    Private Sub insertOrUpdateDETAIL()
        Dim parameters As New Dictionary(Of String, Object)
        Dim query As String = "DELETE FROM FIN_PN_IV_DETAIL WHERE TRAN_NO = @p0"
        parameters.Add("@p0", TRAN_NO)
        ...
        For Each row As DataRow In DETAILDataTable.Rows
            If String.IsNullOrEmpty(row.Item("TRAN_NO").ToString()) Then
                ...
                pnD.PROG_ID = Me.Name
                QueryHelper.insertModel("FIN_PN_IV_DETAIL", pnD)
            Else
                Dim pnD As FIN_PN_IV_DETAIL = CType(ModelHelper.convertDataRowToModel(New FIN_PN_IV_DETAIL, row), FIN_PN_IV_DETAIL)
                pnD.PROG_ID = Me.Name
                QueryHelper.insertModel("FIN_PN_IV_DETAIL", pnD)
    ...
    Private Sub setVATType(inpVATType As String)
        Try
            ...
            If inpVATType = "I" Then
                vatInclude.Checked = True
                setVATTypeRelatedControlStyle(True, False, Color.FromName("ControlLight"), Color.White)
            ElseIf inpVATType = "E" Then
                vatExclude.Checked = True
                setVATTypeRelatedControlStyle(False, True, Color.White, Color.FromName("ControlLight"))
    ...
    End Sub
    Private Sub setVATTypeRelatedControlStyle(U_PRICEReadOnlyState As Boolean, U_PRICE_INC_VATReadOnlyState As Boolean, U_PRICEBackColor As Color, U_PRICE_INC_VATBackColor As Color)
        With DetailGridView
            .Refresh()
            .Columns("U_PRICE").ReadOnly = U_PRICEReadOnlyState
            .Columns("U_PRICE_INC_VAT").ReadOnly = U_PRICE_INC_VATReadOnlyState
            .Columns("U_PRICE").DefaultCellStyle.BackColor = U_PRICEBackColor
            .Columns("U_PRICE_INC_VAT").DefaultCellStyle.BackColor = U_PRICE_INC_VATBackColor
        End With
    End Sub
    ...
    Private Function validateFIN_PN_IV_DETAIL() As Boolean
        Dim retVal As Boolean = True
    ...
    Private Sub PullButton_Click(sender As Object, e As EventArgs)
    ...
    Private Sub updateRC_TRAN_NOPNAndIV(docListString As String)
        Dim parameters As New Dictionary(Of String, Object)
        Dim query As String = "UPDATE FIN_PN_IV_HEAD SET RC_TRAN_NO = @p0 WHERE TRAN_NO IN  (" & docListString & ") "
        parameters.Add("@p0", TRAN_NO)
        Try
            executeWebSQL(query, parameters)
        Catch ex As Exception
    ...
    Private Sub updateDETAIL_BAL_AMT(AMT As Double, TRAN_NO As String, SEQ As Integer)
        Dim parameters As New Dictionary(Of String, Object)
        Dim query As String = "UPDATE FIN_PN_IV_DETAIL SET BAL_AME = @p0 WHERE TRAN_NO = @p1 AND SEQ = @p2"
        parameters.Add("@p0", AMT)
