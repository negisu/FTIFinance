...
Public Class frmMainStatusNew

    Friend MODULE_ID As Integer

    Private Sub OK_Button_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles OK_Button.Click
        If TextBox9.TextLength > 0 And TextBox30.TextLength > 0 And TextBox1.TextLength > 0 Then
            'check existing
            Dim pRep As New Dictionary(Of String, Object)
            pRep.Add("@p0", TextBox1.Text)
            pRep.Add("@p1", MODULE_ID)
            'pRep.Add("@p0", TextBox9.Text)
            'pRep.Add("@p1", TextBox30.Text)
            'pRep.Add("@p2", TextBox1.Text)
            'pRep.Add("@p3", MODULE_ID)

            Dim qRep As String = String.Empty
            qRep &= "SELECT COUNT(*) AS CNT * "
            qRep &= "FROM MEMBER_STATUS_CODE "
            qRep &= "WHERE (MEMBER_STATUS_CODE = @p0) AND (MODULE = @p1)"
            ...
            Catch ex As Exception
                MessageBox.Show(qRep & vbCrLf & vbCrLf & ex.Message, "ERROR=client.ExecuteScalar CNT MEMBER_STATUS_CODE")
            End Try
