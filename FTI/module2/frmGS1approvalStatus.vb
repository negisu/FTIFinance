Public Class frmGS1approvalStatus
    ...
    Private Sub getMB_MEMBER_RETIRE()
        ...
        If RETIRE_TYPE = "R" Then ' release deleted member
            Dim sFTI_STATUS_BLACKLIST_RELEASE() As String = getParameters(2, "GS1_STATUS_BLACKLIST_RELEASE").Split("|"c)
            query &= "WHERE        (DATEDIFF(mm, MB_MEMBER_RETIRE.APPROVE_DATE, GETDATE()) > " & CInt(getParameters(2, "GS1_STATUS_BLACKLIST_RELEASE_AGE")) & ") AND (MB_MEMBER_RETIRE.MEMBER_MAIN_GROUP_CODE IN (" & getParameters(2, "GS1_MAIN_GROUP_APPROVE") & ")) AND (MB_MEMBER_RETIRE.RETIRE_TYPE = '" & sFTI_STATUS_BLACKLIST_RELEASE(0) & "') "
        Else
            query &= "WHERE        (MB_MEMBER_RETIRE.APPROVE_DATE IS NULL) AND (MB_MEMBER_RETIRE.MEMBER_MAIN_GROUP_CODE IN (" & getParameters(2, "GS1_MAIN_GROUP_APPROVE") & ")) AND (MB_MEMBER_RETIRE.RETIRE_TYPE = '" & RETIRE_TYPE & "') "
        End If
        ...
        Label3.Text = String.Format("พบ {0} รายการ", ds.Tables("MB_MEMBER_RETIRE").Rows.Count.ToString("#,##0"))
    End Sub
            ...
            If DataGridView1.SelectedRows.Count > 0 Then
                ...
                For i As Integer = 0 To DataGridView1.SelectedRows.Count - 1
                    ...
                    If RETIRE_TYPE = "R" Then
                        Dim sFTI_STATUS_BLACKLIST_RELEASE() As String = getParameters(1, "GS1_STATUS_BLACKLIST_RELEASE").Split("|"c)
                        'log
                        saveLOGS(DataGridView1.SelectedRows(i).Cells("REGIST_CODE").Value.ToString, "GS1_MAIN", "BLACKLIST_RELEASE", "UPDATE", "", DateTimePicker1.Value.ToString("dd/MM/yyyy", ciTH), user_name)
                        ...
                        'log
                        saveLOGS(DataGridView1.SelectedRows(i).Cells("REGIST_CODE").Value.ToString, "GS1_MAIN", "MEMBER_STATUS_CODE", "UPDATE", getCurrentStatus(DataGridView1.SelectedRows(i).Cells("REGIST_CODE").Value.ToString), sFTI_STATUS_BLACKLIST_RELEASE(1), user_name)

                        'log
                        saveLOGS(DataGridView1.SelectedRows(i).Cells("REGIST_CODE").Value.ToString, "GS1_MAIN", "MEMBER_STATUS_CODE", "UPDATE", DataGridView1.SelectedRows(i).Cells("RETIRE_TYPE").Value.ToString.ToUpper, sFTI_STATUS_BLACKLIST_RELEASE(1), user_name)
                    Else
                        Select Case DataGridView1.SelectedRows(i).Cells("RETIRE_TYPE").Value.ToString.ToUpper
                            Case sFTI_STATUS_MOVE(1).ToUpper
                                ...
                                query = "UPDATE MB_PARAMETERS SET OBJ_VALUE = '" & running + 1 & "' WHERE OBJ_MODULE = 1 AND OBJ_NAME = 'GS1_RUNNING_REGIST'"
