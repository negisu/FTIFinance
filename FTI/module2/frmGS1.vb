...
Public Class frmGS1
    ...
    Private Sub frmFTI_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        'predefine fields
        Dim FTI_MAIN_SEARCH_PRE As String = getParameters(2, "GS1_MAIN_SEARCH_PRE")

        'search as thai text
        Dim FTI_MAIN_SEARCH As String = getParameters(2, "GS1_MAIN_SEARCH")
    ...
    Private Sub getMB_MEMBER_ADVISOR()
        ...
        sFTI_MB_MEMBER_ADVISOR &= "WHERE " & getParameters(2, "GS1_MB_MEMBER_ADVISOR") & " ORDER BY MEMBER_GROUP_NAME"
    ...
    Private Sub getInfo(ByVal REGIST_CODE As String, ByVal COMP_PERSON_CODE As String)
        ...
        'get memcode format
        Dim parameters As New Dictionary(Of String, Object)
        parameters.Add("@p0", tbMEMBER_CODE.Text)
        parameters.Add("@p1", tbMEMBER_TYPE.Text)
        parameters.Add("@p2", tbMEMBER_CODE.Text)

        Dim query As String = String.Empty
        'query = String.Empty
        query = "SELECT LEFT(@p0, LEN(@p0) - LEN(NO_XXXC)) + NO_XXXC AS MEMBER_CODE FROM MB_EAN13 WHERE (IS_EXPAND = 1) AND (MEMBER_TYPE_CODE = @p1) AND (@p2 BETWEEN RANG_FROM AND RANG_TO)"
        'query = "SELECT LEFT(@p0, LEN(@p0) - LEN(NO_XXXC)) + NO_XXXC AS MEMBER_CODE FROM MB_EAN13 WHERE (MEMBER_TYPE_CODE = @p1) AND (@p2 BETWEEN RANG_FROM AND RANG_TO)"

        tbMEMBER_CODE_FORMAT.Text = String.Empty
        Try
            tbMEMBER_CODE_FORMAT.Text = client.ExecuteScalar(query, parameters, user_session).ToString
        Catch ex As Exception
    ...
    Private Sub Button12_Click(sender As Object, e As EventArgs) Handles btNew.Click
        ...
        Try
            ...
            running = CInt(getParameters(2, "GS1_RUNNING_REGIST"))
        Catch ex As Exception
            MessageBox.Show(ex.Message, "ERROR=getParameters=GS1_RUNNING_REGIST")
        End Try

        Dim d As DateTime = getSQLDate()
        Dim REGIST_CODE As String = "G" & d.ToString("yyMM", ciTH) & running.ToString("00000")
        ...
        Dim query As String = String.Empty
        'query = String.Empty
        query = "UPDATE MB_PARAMETERS SET OBJ_VALUE = '" & running + 1 & "' WHERE OBJ_MODULE = 2 AND OBJ_NAME = 'GS1_RUNNING_REGIST'"
        ...
        parameters.Add("@p3", "300")
        parameters.Add("@p4", "300")
        parameters.Add("@p5", "001")
        parameters.Add("@p6", "A12")
        parameters.Add("@p7", d)
                ...
                If MessageBox.Show("ยืนยันที่จะ" & btRepresentAddressSave.Text & "?", "ยืนยัน?", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                    ...
                    For i As Integer = 0 To cols.Length - 1
                        If row.HasVersion(DataRowVersion.Current) = True Then
                            If row(cols(i), DataRowVersion.Original).ToString <> row(cols(i), DataRowVersion.Current).ToString Then
                                'save it
                                saveLOGS(DataGridView5.CurrentRow.Cells("CONTACT_CODE").Value.ToString, "MB_CONTACT", cols(i), "UPDATE", If(IsDBNull(row(cols(i), DataRowVersion.Original)), "", row(cols(i), DataRowVersion.Original)), If(IsDBNull(row(cols(i), DataRowVersion.Current)), "", row(cols(i), DataRowVersion.Current)), user_name)
                            End If
                        End If
                    Next
    ...
    Private Sub btRepresentDelete_Click(sender As Object, e As EventArgs) Handles btRepresentDelete.Click
        ...
        If ispass = True Then
            If MessageBox.Show("ยืนยันที่จะลบ " & TextBox9.Text, "ยืนยัน", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                ...
                MessageBox.Show("ลบผู้ติดต่อเสร็จสิ้น")
            End If
        Else
            MessageBox.Show("กรุณาตรวจสอบกรรมการ วาระปัจจุบัน")
        End If
    ...
    Private Sub btFileAdd_Click(sender As Object, e As EventArgs) Handles btFileAdd.Click
        If tbCOMP_PERSON_CODE.TextLength > 0 Then
            ...
            f.MODULE_ID = 2
            f.DOC_SIZE_PARAMETER = "GS1_DOC_SIZE"
            f.DOC_FILTER_PARAMETER = "GS1_DOC_FILTER"
            If f.ShowDialog(Me) = Windows.Forms.DialogResult.OK Then
                'read file
    ...
    Private Sub LinkLabel1_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles LinkLabel1.LinkClicked
        ...
        If DataGridView1.CurrentRow IsNot Nothing Then
            ...
            'Dim parameters As New Dictionary(Of String, Object)
            ''Dim parameters As New Dictionary(Of String, Object)
            'parameters.Add("@p0", row("MEMBER_MAIN_GROUP_CODE"))
            'parameters.Add("@p1", row("MEMBER_GROUP_CODE"))
            'parameters.Add("@p2", row("MEMBER_MAIN_TYPE_CODE"))
            'parameters.Add("@p3", row("MEMBER_TYPE_CODE"))
            'parameters.Add("@p4", row("REGIST_CAPITAL"))
            'parameters.Add("@p5", CDate(row("REGIST_DATE")).Month)
            ...
            'query &= "                         END_MONTH_FIRST_YEAR) AND (INACTIVE = 'N')"

            'Dim FIRST_REGIST_RATE As Decimal = 0
            'Try
            '    FIRST_REGIST_RATE = CDec(client.ExecuteScalar(query, parameters, user_session))
            'Catch ex As Exception
            '    MessageBox.Show(query & vbCrLf & vbCrLf & ex.Message, "ERROR=client.ExecuteScalar FIRST_REGIST_RATE")
            'End Try
            ...
            ''==============================================
            'query = "SELECT * FROM MB_MEMBER_COMPUTE_DETAIL "
            ''query &= "SELECT [MEMBER_TYPE_CODE], [MEMBER_TYPE_NAME], [INACTIVE] FROM [MB_MEMBER_TYPE] "
            'query &= "WHERE (MEMBER_MAIN_GROUP_CODE = @p0) AND (MEMBER_GROUP_CODE = @p1) AND (MEMBER_MAIN_TYPE_CODE = @p2) AND (MEMBER_TYPE_CODE = @p3) AND (INACTIVE = 'N') "
            'query &= "ORDER BY [RATE_SEQ] "

            'Dim dtCOMPUTE As DataTable = fillWebSQL(query, parameters, "MB_MEMBER_COMPUTE_DETAIL").Copy
            ''dt2.TableName = "MB_MEMBER_COMPUTE_DETAIL"
            'dtCOMPUTE.PrimaryKey = New DataColumn() {dtCOMPUTE.Columns("RATE_SEQ")}
            'If ds.Tables.Contains("MB_MEMBER_COMPUTE_DETAIL") = True Then
            '    ds.Tables("MB_MEMBER_COMPUTE_DETAIL").Clear()
            '    ds.Tables("MB_MEMBER_COMPUTE_DETAIL").Merge(dtCOMPUTE)
            '    ds.Tables("MB_MEMBER_COMPUTE_DETAIL").AcceptChanges()
            '    'ds.Tables.Remove("MB_MEMBER_COMPUTE_DETAIL")
            'End If

            'ds.Tables.Add(dtCOMPUTE)
            ''==============================================
            'Dim dtRULE As DataTable = ds.Tables("MB_MEMBER").Clone
            'dtRULE.ImportRow(row)
            ''==============================================
            ...
            '        RULE_RATE = CDec(ds.Tables("MB_MEMBER_COMPUTE_DETAIL").Rows(i).Item("FIRST_REGIST_VALUE"))
            '        Formula = ds.Tables("MB_MEMBER_COMPUTE_DETAIL").Rows(i).Item("QUERY").ToString

            '        'is it found?
            '        If CInt(dtRULE.Compute("COUNT(MEMBER_CODE)", Formula)) > 0 Then
            '            FIRST_REGIST_RATE = RULE_RATE
            '        End If
            '    Next
    ...
    Private Sub btExpand_Click(sender As Object, e As EventArgs) Handles btExpand.Click
        ...
        If DataGridView1.CurrentRow IsNot Nothing Then
            If tbMEMBER_CODE_FORMAT.TextLength > 0 Then
                'GS1_EXPAND
                Dim PARAMS() As String = getParameters(2, "GS1_EXPAND").Split("|"c)
                If IsExistStatus(PARAMS(0), ComboBox13.SelectedValue.ToString) = True And ComboBox3.SelectedValue.ToString = "300" Then
                    Dim CHK_CODE As String = tbMEMBER_CODE_FORMAT.Text.Replace("C", "")
                    CHK_CODE = CHK_CODE.Replace("X", "")

                    If Microsoft.VisualBasic.Right(CHK_CODE, 1) = "0" Then
                        System.Windows.Forms.Cursor.Current = Cursors.WaitCursor
                        'MB_MEMBER
                        Dim parameters As New Dictionary(Of String, Object)
                        parameters.Add("@p0", tbREGIST_CODE.Text) 'regist code
                        parameters.Add("@p1", ComboBox3.SelectedValue) '1
                        parameters.Add("@p2", ComboBox4.SelectedValue) '2
                        parameters.Add("@p3", ComboBox14.SelectedValue) '3
                        parameters.Add("@p4", ComboBox15.SelectedValue) '4
                        parameters.Add("@p5", DataGridView1.CurrentRow.Cells("COMP_PERSON_CODE").Value) '4
                        parameters.Add("@p6", "E") '4
                        parameters.Add("@p7", getSQLDate()) '4
                        parameters.Add("@p8", "ขอขยายเลขหมาย") '4
                        parameters.Add("@p9", tbMEMBER_STATUS_CODE.Text) '4
                        parameters.Add("@p10", tbMEMBER_CODE.Text) '4

                        Dim query As String = String.Empty
                        query = "INSERT INTO MB_MEMBER_RETIRE (REGIST_CODE, MEMBER_MAIN_GROUP_CODE, MEMBER_GROUP_CODE, MEMBER_MAIN_TYPE_CODE, MEMBER_TYPE_CODE, COMP_PERSON_CODE, RETIRE_TYPE, RETIRE_DATE, RETIRE_REASON, MEMBER_STATUS_CODE, MEMBER_CODE) VALUES (@p0,@p1,@p2,@p3,@p4,@p5,@p6,@p7,@p8,@p9,@p10)"
                        ...
                        'Dim parameters As New Dictionary(Of String, Object)
                        parameters.Clear()
                        parameters.Add("@p0", PARAMS(2).ToUpper)
                        'parameters.Add("@p1", row("MEMBER_MAIN_GROUP_CODE"))
                        'parameters.Add("@p2", row("MEMBER_GROUP_CODE"))
                        'parameters.Add("@p3", row("MEMBER_MAIN_TYPE_CODE"))
                        'parameters.Add("@p4", row("MEMBER_TYPE_CODE"))
                        parameters.Add("@p", tbREGIST_CODE.Text)
                        ...
                        'save it
                        saveLOGS(DataGridView1.CurrentRow.Cells("REGIST_CODE").Value.ToString, "MB_MEMBER", "MEMBER_STATUS_CODE", "UPDATE", tbMEMBER_STATUS_CODE.Text, PARAMS(2).ToUpper, user_name)

                        MessageBox.Show(btExpand.Text & "เรียบร้อย")
                    Else
                        MessageBox.Show("เลขไม่สามารถขยายได้เพราะหลักที่จะขยายมีค่าเป็น " & Microsoft.VisualBasic.Right(CHK_CODE, 1))
                    End If


                Else
                    MessageBox.Show("กรุณาเลือกสมาชิกที่มีสถานะ " & PARAMS(0))
                End If
            Else
                MessageBox.Show("กรุณาเลือกสมาชิกที่มีฟอร์แมตตรงกับผังเลข EAN หรืออยู่ในชุดเลขที่ขยายได้")
            End If
        End If
    End Sub
    ...
    Private Sub btResign_Click(sender As Object, e As EventArgs) Handles btResign.Click
        System.Windows.Forms.Cursor.Current = Cursors.WaitCursor
        If DataGridView1.CurrentRow IsNot Nothing Then
            Dim sFTI_STATUS_RESIGN() As String = getParameters(2, "GS1_STATUS_RESIGN").Split("|"c)
            If IsExistStatus(sFTI_STATUS_RESIGN(0), ComboBox13.SelectedValue.ToString) = True And tbMEMBER_CODE.Text <> "<AUTO>" Then
                Dim f As New frmGS1StatusResignNew
                f.TextBox1.Text = "C"
                f.TextBox2.Text = sFTI_STATUS_RESIGN(1)
                f.REGIST_CODE = tbREGIST_CODE.Text
                If f.ShowDialog(Me) = Windows.Forms.DialogResult.OK Then
                    'MB_MEMBER
                    Dim parameters As New Dictionary(Of String, Object)
                    parameters.Add("@p0", tbREGIST_CODE.Text) 'regist code
                    ...
                    parameters.Add("@p4", ComboBox15.SelectedValue) '4
                    parameters.Add("@p5", DataGridView1.CurrentRow.Cells("COMP_PERSON_CODE").Value) '4
                    parameters.Add("@p6", "C") '4
                    parameters.Add("@p7", getSQLDate()) '4
                    parameters.Add("@p8", f.ComboBox1.Text) '4
                    parameters.Add("@p9", f.TextBox2.Text) '4
                    parameters.Add("@p10", tbMEMBER_CODE.Text) '4
                    ...
                    'Dim parameters As New Dictionary(Of String, Object)
                    parameters.Clear()
                    parameters.Add("@p0", sFTI_STATUS_RESIGN(2).ToUpper)
                    'parameters.Add("@p1", row("MEMBER_MAIN_GROUP_CODE"))
                    'parameters.Add("@p2", row("MEMBER_GROUP_CODE"))
                    'parameters.Add("@p3", row("MEMBER_MAIN_TYPE_CODE"))
                    'parameters.Add("@p4", row("MEMBER_TYPE_CODE"))
                    ...
                    'save it
                    saveLOGS(DataGridView1.CurrentRow.Cells("REGIST_CODE").Value.ToString, "MB_MEMBER", "MEMBER_STATUS_CODE", "UPDATE", tbMEMBER_STATUS_CODE.Text, sFTI_STATUS_RESIGN(2).ToUpper, user_name)

                    MessageBox.Show(btResign.Text & "เรียบร้อย")
                End If
                f.Dispose()
                f = Nothing
            Else
                MessageBox.Show("กรุณาเลือกสมาชิกที่มีสถานะ " & sFTI_STATUS_RESIGN(0))
            End If
        End If
    End Sub

    Private Sub btBlackList_Click(sender As Object, e As EventArgs) Handles btBlackList.Click
        If DataGridView1.CurrentRow IsNot Nothing Then
            Dim sFTI_STATUS_BLACKLIST() As String = getParameters(2, "GS1_STATUS_BLACKLIST").Split("|"c)
            If IsExistStatus(sFTI_STATUS_BLACKLIST(0), ComboBox13.SelectedValue.ToString) = True And tbMEMBER_CODE.Text <> "<AUTO>" Then
                System.Windows.Forms.Cursor.Current = Cursors.WaitCursor
                Dim f As New frmGS1StatusResignNew
                f.TextBox1.Text = "D"
                f.TextBox2.Text = sFTI_STATUS_BLACKLIST(1)
                f.REGIST_CODE = tbREGIST_CODE.Text
                If f.ShowDialog(Me) = Windows.Forms.DialogResult.OK Then
                    'MB_MEMBER
                    Dim parameters As New Dictionary(Of String, Object)
                    ...
                    'Dim parameters As New Dictionary(Of String, Object)
                    parameters.Clear()
                    parameters.Add("@p0", sFTI_STATUS_BLACKLIST(2).ToUpper)
                    'parameters.Add("@p1", row("MEMBER_MAIN_GROUP_CODE"))
                    'parameters.Add("@p2", row("MEMBER_GROUP_CODE"))
                    'parameters.Add("@p3", row("MEMBER_MAIN_TYPE_CODE"))
                    'parameters.Add("@p4", row("MEMBER_TYPE_CODE"))
                    ...
                    'save it
                    saveLOGS(DataGridView1.CurrentRow.Cells("REGIST_CODE").Value.ToString, "MB_MEMBER", "MEMBER_STATUS_CODE", "UPDATE", tbMEMBER_STATUS_CODE.Text, sFTI_STATUS_BLACKLIST(2).ToUpper, user_name)

                    MessageBox.Show(btBlackList.Text & "เรียบร้อย")
                End If
                f.Dispose()
                f = Nothing
            Else
                MessageBox.Show("กรุณาเลือกสมาชิกที่มีสถานะ " & sFTI_STATUS_BLACKLIST(0))
            End If
        End If
    End Sub

    Private Sub btHold_Click(sender As Object, e As EventArgs) Handles btHold.Click
        System.Windows.Forms.Cursor.Current = Cursors.WaitCursor
        If DataGridView1.CurrentRow IsNot Nothing Then
            Dim sFTI_STATUS_HOLD() As String = getParameters(2, "GS1_STATUS_HOLD").Split("|"c)
            If IsExistStatus(sFTI_STATUS_HOLD(0), ComboBox13.SelectedValue.ToString) = True And tbMEMBER_CODE.Text <> "<AUTO>" Then
                Dim f As New frmGS1StatusResignNew
                f.TextBox1.Text = "S"
                f.TextBox2.Text = sFTI_STATUS_HOLD(1)
                f.REGIST_CODE = tbREGIST_CODE.Text
                If f.ShowDialog(Me) = Windows.Forms.DialogResult.OK Then
                    'MB_MEMBER
                    Dim parameters As New Dictionary(Of String, Object)
                    ...
                    'Dim parameters As New Dictionary(Of String, Object)
                    parameters.Clear()
                    parameters.Add("@p0", sFTI_STATUS_HOLD(2).ToUpper)
                    'parameters.Add("@p1", row("MEMBER_MAIN_GROUP_CODE"))
                    'parameters.Add("@p2", row("MEMBER_GROUP_CODE"))
                    'parameters.Add("@p3", row("MEMBER_MAIN_TYPE_CODE"))
                    'parameters.Add("@p4", row("MEMBER_TYPE_CODE"))
                    ...
                    'save it
                    saveLOGS(DataGridView1.CurrentRow.Cells("REGIST_CODE").Value.ToString, "MB_MEMBER", "MEMBER_STATUS_CODE", "UPDATE", tbMEMBER_STATUS_CODE.Text, sFTI_STATUS_HOLD(2).ToUpper, user_name)

                    MessageBox.Show(btHold.Text & "เรียบร้อย")
                End If
                f.Dispose()
                f = Nothing
            Else
                MessageBox.Show("กรุณาเลือกสมาชิกที่มีสถานะ " & sFTI_STATUS_HOLD(0))
            End If
        End If
    End Sub
    ...
    Private Sub btStatus_Click(sender As Object, e As EventArgs) Handles btStatus.Click
        If DataGridView1.CurrentRow IsNot Nothing Then
            Dim sFTI_STATUS_MOVE() As String = getParameters(2, "GS1_STATUS_MOVE").Split("|"c)
            If IsExistStatus(sFTI_STATUS_MOVE(0), ComboBox13.SelectedValue.ToString) = True And tbMEMBER_CODE.Text <> "<AUTO>" And DataGridView1.CurrentRow.Cells("MEMBER_MAIN_GROUP_CODE").Value.ToString = "300" Then
                '
                ...
                If f.ShowDialog(Me) = Windows.Forms.DialogResult.Yes Then
                    ...
                    'new member
                    parameters.Clear()
                    parameters.Add("@p0", DataGridView1.CurrentRow.Cells("MEMBER_CODE").Value)
                    parameters.Add("@p1", sFTI_STATUS_MOVE(0).ToUpper)
                    parameters.Add("@p", f.DataGridView1.CurrentRow.Cells("REGIST_CODE").Value)

                    'save MB_MEMBERS
                    query = String.Empty
                    query = "UPDATE MB_MEMBER SET MEMBER_CODE = @p0, MEMBER_STATUS_CODE = @p1 WHERE REGIST_CODE = @p"
                    ...
                    'save it
                    saveLOGS(f.DataGridView1.CurrentRow.Cells("REGIST_CODE").Value.ToString, "MB_MEMBER", "MEMBER_CODE", "UPDATE", "<PAID>", DataGridView1.CurrentRow.Cells("MEMBER_CODE").Value.ToString, user_name)
                    saveLOGS(f.DataGridView1.CurrentRow.Cells("REGIST_CODE").Value.ToString, "MB_MEMBER", "MEMBER_STATUS_CODE", "UPDATE", "X", sFTI_STATUS_MOVE(0).ToUpper, user_name)

                    '===========================================================================
                    'update old member
                    parameters.Clear()
                    parameters.Add("@p0", sFTI_STATUS_MOVE(1).ToUpper)
                    parameters.Add("@p1", f.DateTimePicker1.Value)
                    parameters.Add("@p2", f.DateTimePicker1.Value)
                    parameters.Add("@p", tbREGIST_CODE.Text)
                    ...
                    'save MB_MEMBERS
                    query = String.Empty
                    query = "UPDATE MB_MEMBER SET MEMBER_STATUS_CODE = @p0, RETIRE_DATE = @p1, APPROVE_RETIRE_DATE = @p2 WHERE REGIST_CODE = @p"
                    ...
                    'save it
                    saveLOGS(DataGridView1.CurrentRow.Cells("REGIST_CODE").Value.ToString, "MB_MEMBER", "MEMBER_STATUS_CODE", "UPDATE", tbMEMBER_STATUS_CODE.Text, sFTI_STATUS_MOVE(1).ToUpper, user_name)
                    saveLOGS(DataGridView1.CurrentRow.Cells("REGIST_CODE").Value.ToString, "MB_MEMBER", "RETIRE_DATE", "UPDATE", tbMEMBER_STATUS_CODE.Text, f.DateTimePicker1.Value.ToString("dd/MM/yyyy", ciTH), user_name)

                    'refresh grid
                    getMEMBER()

                    MessageBox.Show(btStatus.Text & "เรียบร้อย")
                End If
                f.Dispose()
                f = Nothing
            Else
                MessageBox.Show("กรุณาเลือกสมาชิกที่มีสถานะ " & sFTI_STATUS_MOVE(0))
            End If
        End If
    End Sub
