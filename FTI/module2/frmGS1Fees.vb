Public Class frmGS1Fees
    ...
    Private Sub ComboBox2_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ComboBox2.SelectedValueChanged
        ...
        If ComboBox5.SelectedValue IsNot Nothing And ComboBox2.SelectedValue IsNot Nothing Then
            ...
            Dim query3 As String = getParameters(2, "GS1_FEE_RULE2_QUERY")
            Try
                ...
                Else
                    ...
                    dvMEMBER = New DataView(ds.Tables("MB_MEMBER"))
                    DataGridView1.DataSource = dvMEMBER
                    ...
                    For idx As Integer = 0 To DataGridView1.ColumnCount - 1
                        ...
                        Select Case DataGridView1.Columns(idx).ValueType.FullName
                            Case "System.Decimal", "System.Double"
                                DataGridView1.Columns(idx).DefaultCellStyle.Format = "#,##.00"
                    ...
                    'DataGridView1.Columns("STEP1").DefaultCellStyle.Format = "#,##.00"
                    'DataGridView1.Columns("STEP2").DefaultCellStyle.Format = "#,##.00"
                    'DataGridView1.Columns("STEP3").DefaultCellStyle.Format = "#,##.00"
                    'DataGridView1.Columns("STEP4").DefaultCellStyle.Format = "#,##.00"
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        If ds.Tables.Contains("MB_MEMBER") = True Then
            If ds.Tables("MB_MEMBER").Rows.Count > 0 Then
                ...
                'fix null on START_INCOME_AMOUNT and END_INCOME_AMOUNT
                For Each row As DataRow In ds.Tables("MB_MEMBER_RATE_DETAIL").Rows
                    If IsDBNull(row("START_INCOME_AMOUNT")) = True Then row("START_INCOME_AMOUNT") = 1
                    If IsDBNull(row("END_INCOME_AMOUNT")) = True Then row("END_INCOME_AMOUNT") = 99999999999999999
                Next

                Dim dvSTEP1 As New DataView(ds.Tables("MB_MEMBER_RATE_DETAIL"))
                Dim dtSTEP1 As DataTable

                For idx = 0 To ds.Tables("MB_MEMBER").Rows.Count - 1
                    Try
                        dvSTEP1.RowFilter = String.Format("MEMBER_MAIN_TYPE_CODE = '{0}' AND MEMBER_TYPE_CODE = '{1}'", ds.Tables("MB_MEMBER").Rows(idx).Item("MEMBER_MAIN_TYPE_CODE").ToString, ds.Tables("MB_MEMBER").Rows(idx).Item("MEMBER_TYPE_CODE").ToString)
                        dtSTEP1 = dvSTEP1.ToTable

                        YEARLY_RATE = DBNull.Value
                        If IsDBNull(ds.Tables("MB_MEMBER").Rows(idx).Item("INCOME_PER_YEAR")) = False And IsDBNull(ds.Tables("MB_MEMBER").Rows(idx).Item("REGIST_DATE")) = False And dtSTEP1.Rows.Count > 0 Then
                            ...
                            If INCOME_AMOUNT >= 0 Then
                                ...
                                YEARLY_RATE = CDec(dtSTEP1.Compute("SUM(MEMBER_YEARLY_RATE)", Formula))
                        ...
                        End If
                    Catch ex As Exception
                        'MessageBox.Show(ex.Message & vbCrLf & Formula, "ERROR STEP=" & idx & " MEMBER CODE=" & ds.Tables("MB_MEMBER").Rows(idx).Item("MEMBER_CODE").ToString)
                        'Exit Sub
                        'ds.Tables("MB_MEMBER").Rows(idx).Item(
                    End Try
                Next
    ...
    Private Sub TextBox1_TextChanged(sender As Object, e As EventArgs) Handles TextBox1.TextChanged
        dvMEMBER.RowFilter = String.Format("(MEMBER_CODE LIKE '%{0}%') OR (COMP_PERSON_NAME_TH LIKE '%{0}%') OR (COMP_PERSON_NAME_EN LIKE '%{0}%')", TextBox1.Text)
    End Sub
End Class