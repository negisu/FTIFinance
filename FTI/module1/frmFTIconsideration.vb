...
Public Class frmFTIconsideration
    ...
    Private Sub getMB_MEMBER_ADVISOR(ByVal QTY As Integer, ByVal DATE_FROM As Date, ByVal DATE_TO As Date)
        ...
        query &= "WHERE        (MB_MEMBER.MEMBER_DATE BETWEEN @p0 AND @p1) AND (MB_MEMBER.MEMBER_MAIN_GROUP_CODE = '000') AND (MB_MEMBER.ADVISOR_CHECKED = 1) AND (MB_MEMBER.ADVISOR_CODE IS NOT NULL) "
        ...
        Else
            dt.PrimaryKey = New DataColumn() {dt.Columns("MEMBER_MAIN_GROUP_CODE"), dt.Columns("MEMBER_GROUP_CODE"), dt.Columns("MEMBER_MAIN_TYPE_CODE"), dt.Columns("MEMBER_TYPE_CODE"), dt.Columns("ADVISOR_CODE")}
            ds.Tables.Add(dt)
        ...
        query2 &= "WHERE (ACTIVE = 1) AND (MEMBER_MAIN_GROUP_CODE = '000') "
        ...
        'MessageBox.Show(ds.Tables("MB_CONSIDERATION_COMPUTE_DETAIL").Rows.Count.ToString)
        ...
        Try
            For i = 0 To ds.Tables("MB_CONSIDERATION_COMPUTE_DETAIL").Rows.Count - 1
                ...
                'MessageBox.Show(dv.RowFilter & vbCrLf & ro.Rows.Count)

                If ro.Rows.Count > 0 Then
                    For idx = 0 To ds.Tables("MB_MEMBER_ADVISOR").Rows.Count - 1
                        ...
                        'MessageBox.Show(RULE_RATE & vbCrLf & RULE_FORMULA)
                        'MessageBox.Show(ds.Tables("MB_MEMBER_ADVISOR").Rows(idx).Item("ADVISOR_CODE").ToString & vbCrLf & CInt(ro.Compute("COUNT(ADVISOR_CODE)", RULE_FORMULA)))

                        'is it found?
                        If CInt(ro.Compute("COUNT(ADVISOR_CODE)", RULE_FORMULA)) > 0 Then
                            If ro.Columns.Contains("EXP") = False Then
                            ...
                            ds.Tables("MB_MEMBER_ADVISOR").Rows(idx).Item("QTY_VALUE") = CDec(ro.Compute("SUM(EXP)", RULE_FORMULA)) / CInt(ds.Tables("MB_MEMBER_ADVISOR").Rows(idx).Item("QTY"))
        ...
        If ds.Tables("MB_MEMBER_ADVISOR").Rows.Count > 0 Then
            ...
            Select Case ComboBox1.SelectedIndex
                Case 0
                    'report 1
                    parameters.Clear()
                    parameters.Add("@p0", New DateTime(dFROM.Year, dFROM.Month, dFROM.Day, 0, 0, 0))
                    parameters.Add("@p1", New DateTime(dTO.Year, dTO.Month, dTO.Day, 23, 59, 59))

                    Dim query3 As String = String.Empty
                    query3 &= "SELECT        MB_MEMBER.MEMBER_MAIN_GROUP_CODE, MB_MEMBER.MEMBER_GROUP_CODE, MB_MEMBER.MEMBER_MAIN_TYPE_CODE, MB_MEMBER.MEMBER_TYPE_CODE, MB_MEMBER.MEMBER_CODE, MB_PRENAME.PRENAME_TH, MB_COMP_PERSON.COMP_PERSON_NAME_TH, MB_PRENAME.PRENAME_FORMAT_TH, MB_MEMBER.ADVISOR_CODE, "
                    query3 &= "                         MB_MEMBER_GROUP.MEMBER_GROUP_NAME, 0 AS ADVISOR_VALUE, MB_MEMBER.MEMBER_DATE "
                    query3 &= "FROM            MB_MEMBER INNER JOIN "
                    query3 &= "                         MB_COMP_PERSON ON MB_MEMBER.COMP_PERSON_CODE = MB_COMP_PERSON.COMP_PERSON_CODE INNER JOIN "
                    query3 &= "                         MB_MEMBER_GROUP ON MB_MEMBER.ADVISOR_CODE = MB_MEMBER_GROUP.MEMBER_GROUP_CODE LEFT OUTER JOIN "
                    query3 &= "                         MB_PRENAME ON MB_COMP_PERSON.PREN_CODE = MB_PRENAME.PRENAME_CODE "
                    query3 &= "WHERE        (MB_MEMBER.ADVISOR_CHECKED = 1) AND (MB_MEMBER.MEMBER_DATE BETWEEN @p0 AND @p1) AND (MB_MEMBER.MEMBER_MAIN_GROUP_CODE = '000') "
                    query3 &= "ORDER BY MB_MEMBER.MEMBER_DATE, MB_MEMBER.MEMBER_CODE"

                    Dim dt3 As DataTable = fillWebSQL(query3, parameters, "REPORT1").Copy

                    If ds.Tables.Contains("REPORT1") = True Then
                        ds.Tables("REPORT1").Clear()
                        ds.Tables("REPORT1").Merge(dt3)
                        ds.Tables("REPORT1").AcceptChanges()
                    Else
                        ds.Tables.Add(dt3)
                        'dt3.PrimaryKey = New DataColumn() {dt2.Columns("MEMBER_MAIN_GROUP_CODE"), dt2.Columns("MEMBER_GROUP_CODE"), dt2.Columns("MEMBER_MAIN_TYPE_CODE"), dt2.Columns("MEMBER_TYPE_CODE"), dt2.Columns("RATE_SEQ")}
                    End If

                    'MessageBox.Show(ds.Tables("REPORT1").Rows.Count.ToString)

                    'dt.PrimaryKey = New DataColumn() {dt.Columns("MEMBER_MAIN_GROUP_CODE"), dt.Columns("MEMBER_GROUP_CODE"), dt.Columns("MEMBER_MAIN_TYPE_CODE"), dt.Columns("MEMBER_TYPE_CODE"), dt.Columns("ADVISOR_CODE")}

                    'loop to add ADVISOR_VALUE
                    For r As Integer = 0 To ds.Tables("REPORT1").Rows.Count - 1
                        Dim row As DataRow = ds.Tables("MB_MEMBER_ADVISOR").Rows.Find(New Object() {ds.Tables("REPORT1").Rows(r).Item("MEMBER_MAIN_GROUP_CODE"), ds.Tables("REPORT1").Rows(r).Item("MEMBER_GROUP_CODE"), ds.Tables("REPORT1").Rows(r).Item("MEMBER_MAIN_TYPE_CODE"), ds.Tables("REPORT1").Rows(r).Item("MEMBER_TYPE_CODE"), ds.Tables("REPORT1").Rows(r).Item("ADVISOR_CODE")})
                        If row IsNot Nothing Then ds.Tables("REPORT1").Rows(r).Item("ADVISOR_VALUE") = row.Item("QTY_VALUE")
                    Next
                    ...
                    f.ReportViewer1.LocalReport.DataSources.Clear()
                    f.ReportViewer1.LocalReport.DataSources.Add(New Microsoft.Reporting.WinForms.ReportDataSource("dsConsideration1", ds.Tables("REPORT1")))
                ...
                Case 1
                    ...
                    query3 &= "                         MB_PRENAME ON MB_COMP_PERSON.PREN_CODE = MB_PRENAME.PRENAME_CODE "
                    query3 &= "WHERE        (MB_MEMBER.ADVISOR_CHECKED = 1) AND (MB_MEMBER.MEMBER_DATE BETWEEN @p0 AND @p1) AND (MB_MEMBER.MEMBER_MAIN_GROUP_CODE = '000') "
                    query3 &= "ORDER BY MB_MEMBER_GROUP.MEMBER_GROUP_NAME, MB_MEMBER.MEMBER_CODE"
                    ...
                    f.ReportViewer1.LocalReport.ReportPath = System.Environment.CurrentDirectory & "/Reports/module1/rdlc5.3.25.10.FTIconsideration2.rdlc"
