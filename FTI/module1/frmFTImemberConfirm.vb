...
Public Class frmFTImemberConfirm
    ...
    Private Sub getMB_MEMBER_RETIRE(ByVal FROM_DATE As Date, ByVal TO_DATE As Date)
        ...
        parameters.Add("@p0", New Date(FROM_DATE.Year, FROM_DATE.Month, FROM_DATE.Day, 0, 0, 0))
        parameters.Add("@p1", New Date(TO_DATE.Year, TO_DATE.Month, TO_DATE.Day, 23, 59, 59))
        ...
        query &= "                         MB_MEMBER.MEMBER_CODE, MB_MEMBER.REGIST_DATE, MB_MEMBER.MEMBER_DATE, MB_MEMBER.MEMBER_STATUS_CODE, MB_MEMBER.COMP_PERSON_CODE, MB_PRENAME.PRENAME_TH AS PRENAME, "
        ...
        query &= "WHERE        (MB_MEMBER.MEMBER_DATE BETWEEN @p0 AND @p1) AND (MB_COMP_PERSON_ADDRESS.ADDR_CODE = '001') AND (MB_MEMBER_REPRESENT.REPRESENT_SEQ = 1) AND "
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles btPrintPreview.Click
        If DataGridView1.CurrentRow IsNot Nothing Then
            ...
            If fi.ShowDialog(Me) = Windows.Forms.DialogResult.OK Then
                ...
                reportParameters.Add("APPROVE_DATE", fi.DateTimePicker2.Value.ToString("dddd ที่ d MMMM yyyy", ciTH))
                reportParameters.Add("MANAGER_NAME", fi.TextBox2.Text)
                reportParameters.Add("MANAGER_DESIGNATION", fi.TextBox3.Text)
                reportParameters.Add("DOC_USER_NAME", fi.TextBox4.Text)
                reportParameters.Add("DOC_USER_TEL", fi.TextBox5.Text)
                reportParameters.Add("DOC_DATE", fi.DateTimePicker1.Value.ToString("d MMMM yyyy", ciTH))
                reportParameters.Add("PRENAME", DataGridView1.CurrentRow.Cells("PRENAME").Value.ToString)
                reportParameters.Add("CONTACT_FIRST_NAME", DataGridView1.CurrentRow.Cells("CONTACT_FIRST_NAME").Value.ToString)
                reportParameters.Add("CONTACT_LAST_NAME", DataGridView1.CurrentRow.Cells("CONTACT_LAST_NAME").Value.ToString)
                reportParameters.Add("COMP_PERSON_NAME", DataGridView1.CurrentRow.Cells("COMP_PERSON_NAME").Value.ToString)
                reportParameters.Add("MEMBER_TYPE_NAME", DataGridView1.CurrentRow.Cells("MEMBER_TYPE_NAME").Value.ToString)
                reportParameters.Add("CONTACT_PRENAME", DataGridView1.CurrentRow.Cells("CONTACT_PRENAME").Value.ToString)
                ...
                'f.ReportViewer1.LocalReport.ReportPath = System.Environment.CurrentDirectory & "/Reports/module1/rdlc5.3.25.01.FTImemberConfirm.rdlc"
