...
Public Class frmMainMAINTYPE
    ...
    Private Sub getMB_PRENAME()
        ...
        query &= "SELECT MEMBER_MAIN_GROUP_CODE, MEMBER_GROUP_CODE, MEMBER_MAIN_TYPE_CODE, MEMBER_MAIN_TYPE_NAME, INACTIVE "
        query &= "FROM            MB_MEMBER_MAIN_TYPE "
        query &= "WHERE        (MEMBER_MAIN_GROUP_CODE LIKE @p0) OR "
        query &= "                         (MEMBER_GROUP_CODE LIKE @p1) OR (MEMBER_MAIN_TYPE_CODE LIKE @p2) OR (MEMBER_MAIN_TYPE_NAME LIKE @p3) "
        query &= "ORDER BY MEMBER_MAIN_GROUP_CODE, MEMBER_GROUP_CODE, MEMBER_MAIN_TYPE_CODE"

        Dim dt As DataTable = New DataTable

        dt = fillWebSQL(query, parameters, "MB_MEMBER_MAIN_TYPE")
        ...
        Else
            ...
            ds.Tables("MB_MEMBER_MAIN_TYPE").PrimaryKey = New DataColumn() {ds.Tables("MB_MEMBER_MAIN_TYPE").Columns("MEMBER_MAIN_GROUP_CODE"), ds.Tables("MB_MEMBER_MAIN_TYPE").Columns("MEMBER_GROUP_CODE"), ds.Tables("MB_MEMBER_MAIN_TYPE").Columns("MEMBER_MAIN_TYPE_CODE")}

            DataGridView1.DataSource = ds.Tables("MB_MEMBER_MAIN_TYPE")
    ...
    Private Sub btApply_Click(sender As Object, e As EventArgs) Handles btApply.Click
        ...
        If DataGridView1.CurrentRow IsNot Nothing Then
            ...
            If MessageBox.Show("ยืนยันที่จะ" & btApply.Text & "?", "ยืนยัน?", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                Dim row As DataRow = ds.Tables("MB_MEMBER_MAIN_TYPE").Rows.Find(New Object() {DataGridView1.CurrentRow.Cells("MEMBER_MAIN_GROUP_CODE").Value, DataGridView1.CurrentRow.Cells("MEMBER_GROUP_CODE").Value, DataGridView1.CurrentRow.Cells("MEMBER_MAIN_TYPE_CODE").Value})
                ...
                Try
                    ...
                    If ds.Tables("MB_MEMBER_MAIN_TYPE").GetChanges IsNot Nothing Then
                        Try
                            updateWebSQL(query, parameters, ds.Tables("MB_MEMBER_MAIN_TYPE"))
                        Catch ex As Exception
                ...
                Catch ex As Exception
                    MessageBox.Show(ex.Message, "MB_MEMBER_GROUP.UPDATE Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
                End Try
    ...
    Private Sub btDelete_Click(sender As Object, e As EventArgs) Handles btDelete.Click
        ...
        System.Windows.Forms.Cursor.Current = Cursors.WaitCursor
        If DataGridView1.CurrentRow IsNot Nothing Then
            If MessageBox.Show("ยืนยันที่จะลบ " & DataGridView1.CurrentRow.Cells("MEMBER_MAIN_TYPE_NAME").Value.ToString, "ยืนยัน", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                'delete it
                Dim parameters As New Dictionary(Of String, Object)
                ...
                Dim query As String = "DELETE FROM MB_MEMBER_MAIN_TYPE WHERE MEMBER_MAIN_GROUP_CODE = @p0 AND MEMBER_GROUP_CODE = @p1 AND MEMBER_MAIN_TYPE_CODE = @p2"
