...
Public Class frmMainMAINGROUPDESC
    ...
    Private Sub getMB_PRENAME()
        ...
        query &= "SELECT MEMBER_MAIN_GROUP_CODE, MEMBER_GROUP_CODE, MEMBER_GROUP_NAME, MEMBER_GROUP_NAME_EN, MEMBER_GROUP_SHORT_NAME, GROUP_LOCATION, MEMBER_GROUP_RDLC_GROUP, INACTIVE "
        query &= "FROM            MB_MEMBER_GROUP "
        query &= "WHERE        (MEMBER_MAIN_GROUP_CODE LIKE @p0) OR "
        query &= "                         (MEMBER_GROUP_CODE LIKE @p1) OR (MEMBER_GROUP_NAME LIKE @p2) OR (MEMBER_GROUP_NAME_EN LIKE @p3) "
        query &= "ORDER BY MEMBER_MAIN_GROUP_CODE, MEMBER_GROUP_CODE"

        Dim dt As DataTable = New DataTable

        dt = fillWebSQL(query, parameters, "MB_MEMBER_GROUP")
        ...
        Else
            ...
            ds.Tables("MB_MEMBER_GROUP").PrimaryKey = New DataColumn() {ds.Tables("MB_MEMBER_GROUP").Columns("MEMBER_MAIN_GROUP_CODE"), ds.Tables("MB_MEMBER_GROUP").Columns("MEMBER_GROUP_CODE")}

            DataGridView1.DataSource = ds.Tables("MB_MEMBER_GROUP")
            ...
            DataGridView1.Columns("MEMBER_GROUP_NAME_EN").Width = 150
    ...
    Private Sub btApply_Click(sender As Object, e As EventArgs) Handles btApply.Click
        ...
        If DataGridView1.CurrentRow IsNot Nothing Then
            ...
            If MessageBox.Show("ยืนยันที่จะ" & btApply.Text & "?", "ยืนยัน?", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                Dim row As DataRow = ds.Tables("MB_MEMBER_GROUP").Rows.Find(New Object() {DataGridView1.CurrentRow.Cells("MEMBER_MAIN_GROUP_CODE").Value, DataGridView1.CurrentRow.Cells("MEMBER_GROUP_CODE").Value})
                ...
                Try
                    ...
                    If ds.Tables("MB_MEMBER_GROUP").GetChanges IsNot Nothing Then
                        Try
                            updateWebSQL(query, parameters, ds.Tables("MB_MEMBER_GROUP"))
                        Catch ex As Exception
    ...
    Private Sub btDelete_Click(sender As Object, e As EventArgs) Handles btDelete.Click
        ...
        System.Windows.Forms.Cursor.Current = Cursors.WaitCursor
        If DataGridView1.CurrentRow IsNot Nothing Then
            If MessageBox.Show("ยืนยันที่จะลบ " & DataGridView1.CurrentRow.Cells("MEMBER_GROUP_NAME").Value.ToString, "ยืนยัน", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                'delete it
                Dim parameters As New Dictionary(Of String, Object)
                ...
                Dim query As String = "DELETE FROM MB_MEMBER_GROUP WHERE MEMBER_MAIN_GROUP_CODE = @p0 AND MEMBER_GROUP_CODE = @p1"
